...
async function main() {
  ...
  console.log(`✅ Usuários criados: ${admin.email}, ${supervisor.email}, ${operator.email}`);
  ...
  const bancadas: Record<string, string> = {};
  for (const data of bancadasData) {
    const bancada = await prisma.bancada.upsert({
      where: { id: data.id },
      update: {},
      create: { ...data, lastActivity: new Date() },
    });
    bancadas[data.id] = bancada.id;
  }
